	...
	for(index=0;index<archs.length;index++){
		...
		var matchGirlRequest = Request({
			...
			onComplete: function(response){
				...
				var href=archs[i].getAttribute("href");
				var offset = href.indexOf("?");
				var res = response.text.split(',');
				if(res[0] == "False") {
					//console.log("This girl is not noticed. "+href.substring(offset+1)+".");
					if(archs[i].children.length > 0)
						archs[i].children[0].setAttribute("style","box-shadow: 0 14px 0 rgba(128,0,0,0.3);");
					else
						archs[i].setAttribute("style","box-shadow: 7px 4px 5px rgba(128,0,0,0.3);");
				}
				else {
					console.log("This girl has been noticed. "+href.substring(offset+1)+".");
					if(archs[i].children.length > 0) {
						archs[i].children[0].setAttribute("style","box-shadow: 0 14px 0 rgba(0,128,0,0.3);");
						archs[i].children[0].setAttribute("title",res[1]);
						var str = res[1];
						str=str.replace(":","<br/>");
						str=str.replace("|","<br/>").replace("|","<br/>").replace("|","<br/>").replace("|","<br/>");
						archs[i].innerHTML=str+archs[i].innerHTML;
						archs[i].setAttribute("style","font-size:12px;");
					} else {
						archs[i].setAttribute("style","box-shadow: 7px 4px 5px rgba(0,128,0,0.3);");
						archs[i].setAttribute("title",res[1]);
					}
				}
			}
		});
...
function parseGirl(tab){
	...
	//var doc = currentBrowser.contentDocument.documentElement;
	var imgsJS = replaceAll(html.match(ptnImages)[0],"'","");
	console.log(imgsJS);
